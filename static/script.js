...
function updateSidebarList() {
    ...
    // 임시 일기 제외 (id가 'new'이거나 'temp-'로 시작하는 것)
    const savedDiaries = allDiaries.filter(d => d.id !== 'new' && !String(d.id).startsWith('temp-'));
    const reversedDiaries = [...savedDiaries].sort((a, b) => b.date.localeCompare(a.date));
    const currentDate = allDiaries[currentIndex] ? allDiaries[currentIndex].date : null;
    const shouldFade = savedDiaries.length > 7;
