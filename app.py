...
def diary_list():
    ...
    try:
        if date_str:
            rows = conn.execute(
"SELECT id, title, content, sentiment, situation, weather, recommended_url, created_at, date(created_at) as date FROM diaries WHERE date(created_at)=? ORDER BY id DESC LIMIT 200",
                (date_str,)
            ).fetchall()
        else:
            rows = conn.execute(
"SELECT id, title, content, sentiment, situation, weather, recommended_url, created_at, date(created_at) as date FROM diaries ORDER BY id DESC LIMIT 50"
            ).fetchall()
        items = []
        for r in rows:
            item = dict(r)
            # sentiment를 emotion으로도 매핑 (프론트엔드 호환성)
            item['emotion'] = item.get('sentiment', '')
            items.append(item)
    finally:
        conn.close()
